...
const Recommand = () => {
  ...
  const menuItem = menuList[randomIndex];
  console.log(restaurant);
  return (
    <>
      <img src={process.env.PUBLIC_URL + menuItem.imagePath} className="mt-3 w-full h-72 object-cover rounded" />
      <div className="w-full h-80 mt-4 bg-white rounded">
        <p className="text-sm text-textdark m-0 pl-5 pt-4">{menuItem.restaurantType}</p>
        ...
        <p className="text-sm text-textgray m-0 px-3 py-1 pb-2">{menuItem.describe}</p>
        <div className="pl-3">
